...
if (op!=null)
 ...
 if ((op[i]!=',' && op[i]!='/' &&  op[i]!='"' && op[i]!='['&& op[i]!=']' && (op[i]<'a'|| op[i]>'z')&& op[i]!="\\"&& op[i]!='+') || op.length==3 ) {kk++;}
  ...
  return (
    ...
    { <div className={Math.round((ans*100)/qno)!=100  ? "hover:scale-105 flex items-center m-2 justify-between border-purple-300 shadow-lg  font-open-sans border-spacing-1.5  border-2 w-auto rounded-lg box-border p-8" :  " flex items-center m-2 justify-between border-purple-300 shadow-lg  font-open-sans border-spacing-1.5  border-2 w-auto rounded-lg box-border p-8  bg-green-100  " }>
      <img className='w-12 h-12  mr-3 mt-0 align-top' src="https://pluspng.com/img-png/react-logo-png-js-logo-react-react-js-icon-512x512.png"></img>
      <div className=' '>
      <h2 className='text-xl text-amber-600 font-bold  mt-2'>{name}</h2>
      <h4 className='text-bg font-bold text-teal-500 mt-2 mb-1'>Total Questions : {qno}</h4>
